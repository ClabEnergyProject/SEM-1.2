...
def core_model (global_dic, case_dic):
    ...
    var_cost_csp_to_storage = case_dic['VAR_COST_CSP_TO_STORAGE']*numerics_cost_scaling
    var_cost_csp_from_storage = case_dic['VAR_COST_CSP_FROM_STORAGE']*numerics_cost_scaling
...
#     3. Storage to dispatch (when no/low generation) (dispatch_csp_from_storage)
    ...
    if 'CSP' in system_components:
        ...
        dispatch_csp = cvx.Variable(num_time_periods)
        dispatch_csp_to_storage = cvx.Variable(num_time_periods)
        dispatch_csp_from_storage = cvx.Variable(num_time_periods)
        energy_csp_storage = cvx.Variable(num_time_periods) # amount of energy currently stored in CSP storage
        constraints += [
                dispatch_csp >= 0, 
                dispatch_csp <= capacity_csp * solar_series,  ######### might need to be changed to direct solar radiation??

                dispatch_csp_to_storage >= 0, 
                dispatch_csp_to_storage <= capacity_csp_storage / storage_charging_time, ############# storage_charging_time correct???
                dispatch_csp_to_storage <= capacity_csp * solar_series - dispatch_csp, # can only dispatch to storage when there is excess energy generation from csp
                dispatch_csp_from_storage >= 0,
                dispatch_csp_from_storage <= capacity_csp_storage / storage_charging_time, ############# storage_charging_time correct???
                dispatch_csp_from_storage <= energy_csp_storage * (1 - storage_decay_rate), # you can't dispatch more from storage in a time step than is in the battery
                                                                                            # This constraint is redundant
        ...
        fcn2min += capacity_csp * fixed_cost_csp + \
            ...
            cvx.sum(dispatch_csp * var_cost_csp)/num_time_periods + \
            cvx.sum(dispatch_csp_to_storage * var_cost_csp_to_storage)/num_time_periods + \
            cvx.sum(dispatch_csp_from_storage * var_cost_csp_from_storage)/num_time_periods 

        for i in range(num_time_periods):

            constraints += [
                    energy_csp_storage[(i+1) % num_time_periods] == 
                        energy_csp_storage[i] + storage_charging_efficiency * dispatch_csp_to_storage[i] 
                        - dispatch_csp_from_storage[i] - energy_csp_storage[i]*storage_decay_rate
                    ]
    else:
        capacity_csp = 0
        capacity_csp_storage = 0
        dispatch_csp = np.zeros(num_time_periods)
        dispatch_csp_to_storage = np.zeros(num_time_periods)
        dispatch_csp_from_storage = np.zeros(num_time_periods)
...
#---------------------- dispatch energy balance constraint ------------------------------------------
    constraints += [
            ...
            + dispatch_csp
            + dispatch_csp_from_storage
            + dispatch_unmet_demand
            == demand_series + dispatch_to_storage + dispatch_to_pgp_storage + dispatch_csp_to_storage
            ]
...
#    prob.solve(solver = 'GUROBI',BarConvTol = 1e-8, FeasibilityTol = 1e-6)
        ...
        result['DISPATCH_CSP_TO_STORAGE'] = -1 * np.ones(demand_series.size)
        result['DISPATCH_CSP_FROM_STORAGE'] = -1 * np.ones(demand_series.size)
        ...
        if 'CSP' in system_components:
            ...
            result['DISPATCH_CSP'] = np.array(dispatch_csp.value).flatten()/numerics_demand_scaling
            result['CURTAILMENT_CSP'] = result['CAPACITY_CSP'] * solar_series - result['DISPATCH_CSP']
            result['DISPATCH_CSP_TO_STORAGE'] = np.array(dispatch_csp_to_storage.value).flatten()/numerics_demand_scaling
            result['DISPATCH_CSP_FROM_STORAGE'] = np.array(dispatch_csp_from_storage.value).flatten()/numerics_demand_scaling
            ...
            result['DISPATCH_CSP'] = dispatch_csp/numerics_demand_scaling
            ...
            result['DISPATCH_CSP_TO_STORAGE'] = dispatch_csp_to_storage/numerics_demand_scaling
            result['DISPATCH_CSP_FROM_STORAGE'] = dispatch_csp_from_storage/numerics_demand_scaling
